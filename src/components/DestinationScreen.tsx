...
const DestinationScreen: React.FC<DestinationScreenProps> = ({ center }) => {
    ...
    const generateRouteInfo = async (coordRoute: string, routeName: string, storeList: string[], addressList: string[]) => {
        ...
        if (routeInfo) {
            storeRouteInfo(routeInfo, routeName, storeList, addressList);
    ...
    async function getCoordinateForAddresses(addressNamesList: string[]): Promise<{ coordinates: string; matchingPlaceNames: string[]}> {
