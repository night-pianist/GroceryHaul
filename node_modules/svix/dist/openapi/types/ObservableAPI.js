...
class ObservableMessageApi {
    ...
    v1MessageStream(appId, limit, iterator, eventTypes, channels, after, _options) {
        const requestContextPromise = this.requestFactory.v1MessageStream(appId, limit, iterator, eventTypes, channels, after, _options);
        let middlewarePreObservable = rxjsStub_1.from(requestContextPromise);
        for (let middleware of this.configuration.middleware) {
            middlewarePreObservable = middlewarePreObservable.pipe(rxjsStub_2.mergeMap((ctx) => middleware.pre(ctx)));
        }
...
const StreamsApi_1 = require("../apis/StreamsApi");
class ObservableStreamsApi {
    constructor(configuration, requestFactory, responseProcessor) {
        this.configuration = configuration;
        this.requestFactory = requestFactory || new StreamsApi_1.StreamsApiRequestFactory(configuration);
        this.responseProcessor = responseProcessor || new StreamsApi_1.StreamsApiResponseProcessor();
    }
    appendToStream(appId, createStreamIn, idempotencyKey, _options) {
        const requestContextPromise = this.requestFactory.appendToStream(appId, createStreamIn, idempotencyKey, _options);
        let middlewarePreObservable = rxjsStub_1.from(requestContextPromise);
        for (let middleware of this.configuration.middleware) {
            middlewarePreObservable = middlewarePreObservable.pipe(rxjsStub_2.mergeMap((ctx) => middleware.pre(ctx)));
        }
        return middlewarePreObservable.pipe(rxjsStub_2.mergeMap((ctx) => this.configuration.httpApi.send(ctx))).
            ...
            return middlewarePostObservable.pipe(rxjsStub_2.map((rsp) => this.responseProcessor.appendToStream(rsp)));
        }));
    }
    listStream(appId, _options) {
        const requestContextPromise = this.requestFactory.listStream(appId, _options);
        let middlewarePreObservable = rxjsStub_1.from(requestContextPromise);
        for (let middleware of this.configuration.middleware) {
            middlewarePreObservable = middlewarePreObservable.pipe(rxjsStub_2.mergeMap((ctx) => middleware.pre(ctx)));
        }
        return middlewarePreObservable.pipe(rxjsStub_2.mergeMap((ctx) => this.configuration.httpApi.send(ctx))).
            ...
            return middlewarePostObservable.pipe(rxjsStub_2.map((rsp) => this.responseProcessor.listStream(rsp)));
        }));
    }
}
exports.ObservableStreamsApi = ObservableStreamsApi;
