...
function isComponentDirectory(ctx, directory, isRoot) {
  ...
  const filename = isRoot ? import_constants.ROOT_DEFINITION_FILENAME : import_constants.DEFINITION_FILENAME;
  const definitionPath = import_path.default.resolve(import_path.default.join(directory, filename));
...
async function buildComponentDirectory(ctx, definitionPath) {
  const isRoot = import_path.default.basename(definitionPath) === import_constants.ROOT_DEFINITION_FILENAME;
  ...
  if (isComponent.kind === "err") {
    (0, import_context.logError)(
      ctx,
      `Invalid component directory (${isComponent.why}): ${import_path.default.dirname(definitionPath)}`
    );
...
function toAbsolutePath(rootComponent, componentDefinitionPath) {
  return import_path.default.normalize(import_path.default.join(rootComponent.path, componentDefinitionPath));
}
