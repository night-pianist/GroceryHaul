...
__export(component_server_exports, {
  componentServerDTS: () => componentServerDTS,
  componentServerJS: () => componentServerJS,
  componentServerStubDTS: () => componentServerStubDTS
});
...
function componentServerJS(isRoot) {
  let result = `
...
function componentServerDTSPrelude() {
  return `
    ${(0, import_common.header)(
    "Generated utilities for implementing server-side Convex query and mutation functions."
  )}
...
function componentServerStubDTS(isRoot) {
...
async function componentServerDTS(ctx, startPush, rootComponent, componentDirectory) {
  ...
  if (!analysis) {
    (0, import_context.logError)(ctx, `No analysis found for component ${definitionPath}`);
...
async function resolveFunctionReference(ctx, analysis, reference) {
  if (!reference.startsWith("_reference/function/")) {
    (0, import_context.logError)(ctx, `Invalid function reference: ${reference}`);
    return await ctx.crash(1, "fatal");
  }
  ...
  if (!analyzedModule) {
    (0, import_context.logError)(ctx, `Module not found: ${modulePath}`);
    return await ctx.crash(1, "fatal");
  }
  ...
  if (!analyzedFunction) {
    (0, import_context.logError)(ctx, `Function not found: ${functionName}`);
    return await ctx.crash(1, "fatal");
  }
  ...
  if (argsValidator) {
    if (argsValidator.type === "object" || argsValidator.type === "any") {
      ...
      (0, import_context.logError)(ctx, `Invalid function args: ${analyzedFunction.args}`);
      return await ctx.crash(1, "fatal");
    }
  }
